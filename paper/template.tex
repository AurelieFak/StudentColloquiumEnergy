...
Little work has been done in the design of eco-feedback systems. However, the researchers in the domain ( Human Computer Interface designers and environmental psychologists) agree that components such as historical comparison, presentation of costs and disaggregation by appliance are the most effective ones, even though there are no universal rules.
...
\subsection{Numerical data : choice of units}
The data provided to the users has to be understandable and familiar. The designers can present the energy consumption with scientific units such as : kWh (usually used in the bill), kgCO2,.., using a monetary unit, like euros or environmental units such as tree saved.
To make it attractive for users, some designers do not lack in originality in their choice of units for displayed the amount of consumed energy. For example, an exposition in the Bernoulliborg by ScienceLinX shows an ECF that displays the consumption in the units ‘hamburgers or coffees bought’.
...
In order to have more insight/more information about what young potential users want we conducted a survey that we posted on social networks: Facebook and Twitter.
The 31 respondents were aged from 16 to 26.%\footnote{The results are available online at \url{}}

First, we learnt that most of the participants (54,5\%) want to save energy but do not know how to accomplish this and that they are unaware of the existence of eco-feedback systems (64.5\%).
 
Not surprisingly, at the 2.0 era, young people want an app (80\%) which could either be a classical ECF system (45.2\%) or a game (54.8\%).
They prefer graphs such as pie charts and bar charts (87.9\%) rather than text or table. 
Moreover, compared to historical comparison and presentation of costs, disaggregation by appliance (52.2\%) is favoured by the participants.
When asked if they would be more motivated to save energy if they knew how much energy their peers use, about half of them replied that they did not mind and half of them said it would motivate them.
The respondents indicated that money is the most understandable unit for them.
